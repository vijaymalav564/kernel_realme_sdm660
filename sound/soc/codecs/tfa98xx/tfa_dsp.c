...
/*
 * This is the right first time to probe and read the IC version, thus to determine if it's TFA9890 or TFA9891,
 * and then select correct the container file to load
 */
enum Tfa98xx_Error
tfa_preFirstTime_probe(unsigned char slave_address, Tfa98xx_handle_t *pHandle)
{
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    int rev;
    int idx = 0;
...
#if 1// (defined(TFA9887)  || defined (TFA9887B) || defined(TFA98XX_FULL))
...
/*enum Tfa98xx_Error tfa98xx_get_volume_level(Tfa98xx_handle_t handle, unsigned short *vol)
{
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
...
{
    enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
    pr_info("coldstart%s :", force? " (forced)":"");
...
enum Tfa98xx_Error tfaRunSpeakerCalibration(Tfa98xx_handle_t handle, int profile)
{
    ...
    if(err == Tfa98xx_Error_Ok) {
        ...
        if (spkr_count == 1) {
            ...
            {
                if((handles_local[handle].mohm[0] < 6000) || (handles_local[handle].mohm[0] > 10500))//6ohm - 10.5ohm
                {
                    pr_info("speaker_component_fail\n");
...
enum Tfa98xx_Error tfaRunStartup(Tfa98xx_handle_t handle, int profile)
{
	...
	/*Reset Coolflux */
	 switch (tfa98xx_dev_revision(handle)) {
		case 0x80:
			pr_info("%s: need to reset DSP here for tfa9890\n", __func__);
...
enum tfa_error tfa_start(int next_profile, int *vstep)
{
    ...
    for( dev=0; dev < devcount; dev++) {
        ...
        if(ftm_mode == BOOT_MODE_FACTORY)
        {
            ...
            {
                err = tfaRunSpeakerBoost(dev, 1, next_profile);
                Calibration_count++;
            }
            if ( err != Tfa98xx_Error_Ok)
            {
...
tfa_dsp_get_calibration_impedance(Tfa98xx_handle_t handle)
{
    ...
    if (error == Tfa98xx_Error_Ok) {
        ...
        if (TFA_GET_BF(handle, MTPOTC) && ((handles_local[handle].rev & 0xff) == 0x88)) {
            if (tfa98xx_runtime_verbose)
                pr_info("Getting calibration values from MTP\n");
            ...
            if (tfa98xx_runtime_verbose)
                pr_info("Getting calibration values from Speakerboost\n");
