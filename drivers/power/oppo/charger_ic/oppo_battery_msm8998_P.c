...
#ifdef CONFIG_PRODUCT_REALME_RMX1801
    /* Ji.Xu@SW.BSP.CHG, 2018-9-3 add for recover charging while plug in/out */
...
#ifdef CONFIG_PRODUCT_REALME_RMX1801//Fanhong.Kong@PSW.BSP.CHG,add 2017/12/02 for shipmode  stm6620
    if (g_oppo_chip) {
	...
	union power_supply_propval *val)
{
	...
	if (g_oppo_chip->charger_exist) {
		if (g_oppo_chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
...
static int oppo_chg_get_shutdown_soc(void)
{
		...
		if (!g_oppo_chip || !chg || !chg->bms_psy) {
