...
static struct silfp_data *optical_fp = NULL;
...
int silfp_opticalfp_irq_handler(struct fp_underscreen_info* tp_info)
{
    ...
    wake_lock_timeout(&optical_fp->wakelock, 1*HZ);
...
static int silfp_probe(struct platform_device *spi)
{
	...
	if (FP_SILEAD_OPTICAL_70 != get_fpsensor_type()) {
			LOG_MSG_DEBUG(INFO_LOG, "[%s] not  silfp fingerprint.\n", __func__);
			status = -EINVAL;
			return status;
	}
...
EXPORT_SYMBOL(silfp_opticalfp_irq_handler);
