...
return:
    Executive outcomes. 0---succeed. -5---access fail.
*******************************************************/
static int32_t nvt_set_page(struct chip_data_nt36672 *chip_info, uint32_t addr)
{
    uint8_t buf[4] = {0};
...
static int32_t nvt_get_fw_info_noflash(struct chip_data_nt36672 *chip_info)
{
        uint8_t buf[64] = {0};
...
info_retry:
        ...
        TPD_DETAIL("fw_ver = 0x%x, fw_sub_ver = 0x%x\n", chip_info->fw_ver, chip_info->fw_sub_ver);
...
return:
        Executive outcomes. 0---succeed. -5---access fail.
*******************************************************/
static void nvt_boot_ready(struct chip_data_nt36672 *chip_info, uint8_t ready)
{
...
return:
        n.a.
*******************************************************/
static void nvt_eng_reset(struct chip_data_nt36672 *chip_info)
{
...
int32_t nvt_nf_detect_chip(struct chip_data_nt36672 *chip_info)
{
    int32_t ret = 0;
    int i;
    uint8_t buf[8] = {0};
...
static int nvt_enable_edge_limit(struct chip_data_nt36672 *chip_info, int state)
{
    int8_t ret = -1;
    struct touchpanel_data *ts = spi_get_drvdata(chip_info->s_client);
