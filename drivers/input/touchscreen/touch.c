...
int tp_util_get_vendor(struct hw_resource *hw_res, struct panel_info *panel_data)
{
   ...
   if (is_project(OPPO_16051)) {
       ...
       pr_err("[TP]%s: fw_name = %s \n",__func__, panel_data->fw_name);
   } else if (is_project(OPPO_18001) || is_project(OPPO_18002)) {
       panel_data->test_limit_name = kzalloc(sizeof(S3706_BASELINE_TEST_LIMIT_NAME_18001), GFP_KERNEL);
       if (panel_data->test_limit_name == NULL) {
           pr_err("[TP]panel_data.test_limit_name kzalloc error\n");
           return -1;
       }
       strcpy(panel_data->test_limit_name, S3706_BASELINE_TEST_LIMIT_NAME_18001);
       strcpy(panel_data->fw_name, S3706_FW_NAME_18001);
       pr_err("[TP]%s: fw_name = %s \n",__func__, panel_data->fw_name);
